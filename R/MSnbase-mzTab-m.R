...
## Accessors
##r <- unlist(m@Metadata[paste("assay[",1:72,"]", sep="")])
...
#  m <- MzTab("/vol/bioinvindex/Submissions/MIC044/MTBLS1582/Height_SMpos_202047957.mzTab")
...
MzTab <- function(file) {
   ...
   ## from each line.  Though is it important to record the
    ...
    res <- setNames(
        ...
        c("Metadata", "Proteins", "Peptides", "PSMs", "SmallMolecules", "MoleculeFeatures", "MoleculeEvidence"))
...
makeAns <- function(object,
                           smabundance = "abundance_assay") {
    x <- smallMolecules(object)
    if (nrow(x) == 0) {
        ans <- new("XCMSnSet")
    } else {
        ecols <- grep(smabundance, names(x))
        e <- as.matrix(x[, ecols])
        if (length(ecols) > 0) {
            fd <- x[, -ecols]
        } else {
        ...
        rownames(e) <- rownames(fd) <-
            make.names(fd[, "SML_ID"], unique = TRUE)
        pd <- data.frame(row.names = colnames(e))
        ans <- list(exprs = e, fData = fd, pData = pd)
    }
# r <- unlist(m@Metadata[paste("assay[",1:72,"]", sep="")])    
